...
- **Regex-based `--faker-types` (#174)** — a `regex:`-prefixed value in a `--faker-types` YAML registers a Datafaker `regexify(pattern)` generator (e.g. `iso_msg_id: "regex:[A-Z0-9]{10,35}"`), so structures can declare patterned fields (structured IDs, ISO references) without code. Invalid patterns fail fast at load; generation stays deterministic
