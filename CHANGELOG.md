...
- **Full performance re-run (14 Jul 2026)** — every JMH and E2E benchmark re-measured against current `main`; all perf docs refreshed from the raw results, archived in `benchmarks/results-2026-07-14/`. End-to-end throughput is unchanged (~32–39K rec/s file, ~21–31K Kafka), but several published claims did not survive:
  ...
  - **Thread scaling is real but structure-dependent, and the E2E harness was hiding it**: `run_e2e_test.sh` times the whole CLI process, so ~1.5–1.7s of fixed JVM + Datafaker-locale startup — about half the wall clock of a 100K-record run — swamps any generation speedup. On the engine's own clock, 8 threads vs 1 gives **2.3×** (nested invoice → file), **1.4×** (passport → file), **1.3×** (Kafka), **1.0×** (primitives, already writer-bound). Only generation and serialization are parallel; the single writer thread is serial. The old "3.7× / 92% efficiency" claim was still overstated, and the E2E tables now carry an explicit warning about what they measure
  - **Kafka scales worst (~1.3×) because compression runs on the writer thread**: `KafkaProducer.send()` compresses each record into its batch buffer *on the calling thread*, and that call is made from the single writer thread — so snappy compression is serialized onto one core while the workers idle. Measured (nested invoice → Kafka, engine-only): `snappy` 28.8K→32.9K rec/s at 1→4 threads, vs `compression: none` 35.9K→**54.7K** (**+66% at 4 threads**). `KafkaProducer` is thread-safe and SeedStream's global record ordering is meaningless for Kafka (null keys → round-robin partitioning), so letting workers `send()` directly should recover most of the gap — not yet done
