...
public class ExecuteCommand implements Callable<Integer> {
   ...
   * objects and represents arrays as pipe-delimited strings. Protobuf and Avro provide compact
   * binary encoding (typically 50-70% smaller than JSON).
   *
   * <p>Default: {@code json}
   *
   ...
   * @see com.datagenerator.formats.avro.AvroSerializer
   */
  @Option(
      names = {"-f", "--format"},
      description = "Output format: json, csv, protobuf, avro, avro-registry, cbeff (default: json)",
      defaultValue = "json")
  ...
  public Integer call() throws Exception {
    ...
    log.info(
        "Time elapsed: {} ms ({} records/sec)",
        elapsed,
        String.format(Locale.ROOT, "%.2f", recordsPerSec));
